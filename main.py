...
class 硬件环境:
    ...
    def 默认配置(self):
        ...
        return {"轮询毫秒":轮询,"停止键":"Key.esc","记录时长秒":记录,"数据文件":f"records_{指纹}.json","序列长度":序列,"最小重复次数":重复,"播放循环":True,"实时同步毫秒":同步,"鼠标移动持续毫秒":鼠标,"最大样本":样本,"硬件指纹":指纹}
class 配置:
...
class 事件记录器:
    ...
    def 记录(self,信息):
        ...
        return True
    def 采集(self):
        print("正在即时记录您的游戏操作，请立即开始游戏，按停止键结束。")
